...
const AGENTCORE_RUNTIME_ARN = process.env.AGENTCORE_RUNTIME_ARN || "arn:aws:bedrock-agentcore:us-west-2:975050325676:runtime/agentcore_app5-hzJtlH2rgJ"; // AgentCore runtime ARN
const AGENTCORE_SESSION_ID = `session-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`; // 33+ char session ID
const AGENTCORE_URL = process.env.AGENTCORE_URL; // 例: http://agentcore:8080/invoke
...
${extracted}`
    ...
    // If AGENTCORE_AGENT_ID is set and caller provided structured params, call the AgentCore service
    const target = body.target; // e.g. "高校生"
    const difficulty = body.difficulty; // e.g. "普通"
    const num_questions = body.num_questions;

    if (AGENTCORE_RUNTIME_ARN && target && difficulty && (typeof num_questions !== "undefined")) {
      try {
        const agentPayload = {
        ...
        const agentResp = await agentRuntime.send(new InvokeAgentCommand({
          agentRuntimeArn: AGENTCORE_RUNTIME_ARN,
          runtimeSessionId: AGENTCORE_SESSION_ID,
          payload: JSON.stringify(agentPayload, null, 2)
        }));

        // AgentCore レスポンス：response body から JSON を抽出
        let agentResult = "";
        const responseBody = agentResp.response;
        if (responseBody) {
          agentResult = await responseBody.transformToString();
        }

        // JSON または直接文字列応答として返す
        let parsedResult = agentResult;
        try {
          parsedResult = JSON.parse(agentResult);
        } catch {
          // プレーンテキスト応答をそのまま使用
        }

        return res(200, { extractedPreview: extracted.slice(0, 400), quiz: parsedResult });
      } catch (e) {
        console.warn("AgentCore invocation failed, falling back to local generation:", String(e));
      }
    }

    // Fallback: ローカルの Bedrock による問題生成
